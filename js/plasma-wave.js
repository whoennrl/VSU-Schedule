...
var F='precision highp float;\nuniform vec2  u_res;\nuniform float u_time;\nuniform vec3  u_color;\nuniform vec3  u_bg;\nuniform float u_speed;\nuniform float u_intensity;\nfloat h21(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453);}\nfloat vn(vec2 p){vec2 i=floor(p),f=fract(p),u=f*f*(3.0-2.0*f);return mix(mix(h21(i),h21(i+vec2(1,0)),u.x),mix(h21(i+vec2(0,1)),h21(i+vec2(1,1)),u.x),u.y);}\nfloat blobRadius(float phi, float t){\n  float r=0.44;\n  r+=sin(phi*1.0+t*0.17)*0.06;\n  r+=sin(phi*3.0+t*0.29)*0.05;\n  r+=sin(phi*2.0-t*0.11)*0.035;\n  r+=0.03*sin(t*0.19)+0.02*sin(t*0.13+1.1);\n  return clamp(r,0.30,0.60);\n}\nvoid main(){\n  vec2 uv=gl_FragCoord.xy/u_res;\n  vec2 p=(uv*2.0-1.0)*vec2(u_res.x/u_res.y,1.0);\n  float t=u_time*u_speed;\n  float r=length(p);\n  float phi=atan(p.y,p.x);\n  float eps=0.18;\n  float br=(blobRadius(phi,t)+blobRadius(phi+eps,t)+blobRadius(phi-eps,t))/3.0;\n  float sd=r-br;\n  float outerMask=smoothstep(-0.04,0.04,sd);\n  float rim=(1.0-outerMask)*exp(sd*14.0)*0.32;\n  float w1=pow(max(sin(sd*20.0-t*2.3),0.0),2.0)*exp(-sd*sd*7.0);\n  float w2=pow(max(sin(sd*11.0-t*1.4+2.1),0.0),2.5)*exp(-sd*sd*4.5)*0.55;\n  float w3=pow(max(sin(sd*5.5-t*0.8+4.2),0.0),3.0)*exp(-sd*sd*2.5)*0.3;\n  float waves=(w1+w2+w3)*outerMask*u_intensity;\n  float sh=vn(vec2(phi*1.8+t*0.45,sd*3.0+t*0.18));\n  waves*=(0.5+sh*0.9);\n  float fade=1.0-clamp(sd*2.8,0.0,1.0);\n  float glow=clamp(waves+rim,0.0,1.0);\n  vec3 col=mix(u_bg,u_color*fade,glow);\n  float vr=dot(uv*2.0-1.0,uv*2.0-1.0);\n  col*=1.0-clamp(vr*0.2,0.0,0.45);\n  gl_FragColor=vec4(clamp(col,0.0,1.0),1.0);\n}';
...
Inst.prototype._b=function(){
  ...
  c.style.zIndex=String(this.o.zIndex);
  c.style.opacity=String(this.o.opacity);
  ...
  this._r();
  this._ro=new ResizeObserver(function(){me._r();});this._ro.observe(el);
};
Inst.prototype._r=function(){
  ...
  var pw=Math.ceil(w*d);var ph=Math.ceil(h*d);
  if(this.c.width===pw&&this.c.height===ph)return;
